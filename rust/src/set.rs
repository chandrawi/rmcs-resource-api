...
pub mod set_service_server {
    ...
    #[async_trait]
    pub trait SetService: Send + Sync + 'static {
        async fn read_set(
            &self,
    ...
    #[derive(Debug)]
    pub struct SetServiceServer<T: SetService> {
        inner: Arc<T>,
    ...
    impl<T: SetService> SetServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: SetService> Clone for SetServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: SetService> tonic::server::NamedService for SetServiceServer<T> {
        const NAME: &'static str = "set.SetService";
    }
}
